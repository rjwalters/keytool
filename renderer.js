...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const walletUI = new WalletUI(walletContainer, handleWalletDataChange, handleGenerateShares);

    const new128BitWalletBtn = document.querySelector('button[id="new-128-bit-wallet-btn"]');
    const new256BitWalletBtn = document.querySelector('button[id="new-256-bit-wallet-btn"]');
    ...
    async function handleGenerateShares(minimum, total) {
        if (minimum >= total || minimum < 2 || total < 3) {
            walletUI.showError('Invalid share configuration. Ensure minimum is less than total and both are at least 2 and 3 respectively.');
            return;
        }
